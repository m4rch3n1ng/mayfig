use super::{FLAGS_FIELD, NAME, PATTERN_FIELD};
...
impl<'de> MapAccess<'de> for RegexDeserializer<'de, '_> {
		...
		K: serde_core::de::DeserializeSeed<'de>,
	{
		if self.regex.is_some() {
			seed.deserialize(BorrowedStrDeserializer::new(PATTERN_FIELD))
				.map(Some)
...
impl<'de> VisitMap {
	...
	pub fn next_key_seed<V: serde_core::de::MapAccess<'de>>(
		...
		let result = if let Some(key) = key {
			...
			let pattern = visitor.next_value::<String>()?;
			let (k, flags) = visitor.next_entry::<&str, String>()?.unwrap();
			// TODO: fail more gracefully
			assert_eq!(k, FLAGS_FIELD);
